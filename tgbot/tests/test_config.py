...
def test_settings_from_env_uses_catalog_override(monkeypatch) -> None:
    ...
    assert settings.catalog_api_url == "http://127.0.0.1:8000/api/restaurant/variants/"
    assert settings.catalog_refresh_interval_seconds == 300
    assert settings.catalog_http_timeout_seconds == 5
